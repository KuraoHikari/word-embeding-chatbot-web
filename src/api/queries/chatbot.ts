...
import type { ApiError, ApiResponse, Chatbot, ChatbotDetail, CreateChatbotPayload, UpdateChatbotPayload } from "../types";
...
// Get chatbot details with stats (for detail page)
export const useGetChatbotDetail = (id: number, enabled = true) => {
 return useQuery<ChatbotDetail, ApiError>({
  queryKey: [...chatbotKeys.detail(id), "stats"],
  queryFn: async () => {
   const response = await apiClient.get(endpoints.chatbots.getById(id));
   return response.data;
  },
...
export const useCreateChatbot = () => {
 ...
 return useMutation<ApiResponse<Chatbot>, ApiError, CreateChatbotPayload>({
  mutationFn: async (payload) => {
   ...
   const response = await apiClient.post<ApiResponse<Chatbot>>(endpoints.chatbots.create, formData, {
    headers: {
...
export const useUpdateChatbot = (id: number) => {
 ...
 return useMutation<ApiResponse<{ message: string }>, ApiError, UpdateChatbotPayload>({
  mutationFn: async (payload) => {
   ...
   const response = await apiClient.patch<ApiResponse<{ message: string }>>(endpoints.chatbots.update(id), formData, {
    headers: {
     "Content-Type": "multipart/form-data",
    },
   });
...
export const useDeleteChatbot = () => {
 ...
 return useMutation<ApiResponse<{ message: string }>, ApiError, number>({
  mutationFn: async (id) => {
   const response = await apiClient.delete<ApiResponse<{ message: string }>>(endpoints.chatbots.delete(id));
   return response.data;
  },
  onSuccess: () => {
   // Invalidate chatbots list to refetch
